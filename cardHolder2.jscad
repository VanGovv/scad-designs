function main() {
  ...
  // return union(cardHolder(config), cardHolderCover(config).translate([-1.25, -1.25, -1]));
  return cardHolderCover(config).translate([-1.25, -1.25, -1]);
}
/*-------------------------------------------------------------------------------------------------
...
function cardHolder({
  ...
  let upperCutout = difference(
    cube([holderHeight - coverOverhang, holderHeight - coverOverhang, cardWidth + 2 * wallSize]),
  ...
  let single = difference(
    ...
    cube([cardWidth, cardHeight + wallSize, holderHeight - floorStrength]).translate([
      wallSize,
      0,
      floorStrength,
    ]),
    floorCutout(cardWidth, floorStrength).translate([cardWidth / 3, 0, 0]),
    upperCutout
  );
...
function cardHolderCover({
  ...
  let base = difference(
    cube([coverWidth, cardHeight + wallSize * 3 + clearance, holderHeight + wallSize]),
    cube([
  ...
  cutouts.push(
    upperCutout(holderHeight, coverWidth, coverOverhang).translate([0, wallSize, wallSize])
  );

  for (let i = 0; i < count; i++) {
    cutouts.push(
      floorCutout(cardWidth, floorStrength).translate([
    ...
    cutouts.push(
      cube([(2 * cardWidth) / 3, wallSize, coverOverhang + wallSize]).translate([
...
function floorCutout(cardWidth, floorStrength) {
  return union(
    cube([cardWidth / 3, cardWidth / 6, floorStrength]),
    cylinder({ d: cardWidth / 3, h: floorStrength }).translate([cardWidth / 6, cardWidth / 6, 0]),
    difference(
      cube([cardWidth / 6, cardWidth / 6, floorStrength]),
      cylinder({ r: cardWidth / 6, h: floorStrength }).translate([0, cardWidth / 6, 0])
    ).translate([-cardWidth / 6, 0, 0]),
    difference(
      cube([cardWidth / 6, cardWidth / 6, floorStrength]),
      cylinder({ r: cardWidth / 6, h: floorStrength }).translate([cardWidth / 6, cardWidth / 6, 0])
...
function upperCutout(holderHeight, coverWidth, coverOverhang) {
  return difference(
    cube([holderHeight - coverOverhang, holderHeight - coverOverhang, coverWidth]),
