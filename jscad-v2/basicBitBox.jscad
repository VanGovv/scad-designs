...
const basicBitBox = ({
    ...
    if (magnetDiameter && magnetHeight && includeMagnets.length > 0) {
        ...
        if (includeMagnets.includes("TNE") || includeMagnets.includes("FNE")) magnetColumns.push(translate([x / 2 - offset, -y / 2 + offset, 0], magnetColumn("ne")));
        if (includeMagnets.includes("TSE") || includeMagnets.includes("FSE")) magnetColumns.push(translate([-x / 2 + offset, -y / 2 + offset, 0], magnetColumn("se")));
        if (includeMagnets.includes("TNW") || includeMagnets.includes("FNW")) magnetColumns.push(translate([x / 2 - offset, y / 2 - offset, 0], magnetColumn("nw")));
        if (includeMagnets.includes("TSW") || includeMagnets.includes("FSW")) magnetColumns.push(translate([-x / 2 + offset, y / 2 - offset, 0], magnetColumn("sw")));
    }
